...
### Stale Skills
- Read `.cursor/skill-usage.json` (if it exists) to get `last_used` and `used_count` per skill
- Read `.cursor/rules/skill-index.mdc` to get the full list of skill names
- For each skill in skill-index.mdc:
...
4. If in "apply" mode, perform cleanup actions:
   ...
   - Stale/unused skills: ask user which to archive, then remove from `.cursor/rules/skill-index.mdc`
