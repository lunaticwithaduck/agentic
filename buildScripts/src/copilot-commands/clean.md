...
### Stale Skills
- Read `.github/skill-usage.json` (if it exists) to get `last_used` and `used_count` per skill
- Read `.github/skills/skill-rules.json` to get the full list of skill names
...
4. If in "apply" mode, perform cleanup actions:
   ...
   - Stale/unused skills: ask user which to remove, then delete the skill directory from
     `.github/skills/` and remove its entry from `.github/skills/skill-rules.json`
