...
8. Skill Candidating — MANDATORY, cannot be skipped silently:
   ...
   **Skill candidate evaluation:**
   ...
   - After writing the `.sc` file, check the synthesis threshold:
     ...
     2. Check whether `.github/skills/[domain]/SKILL.md` already exists
     3. If count ≥ 3 AND no skill file exists yet: **synthesize the skill now, in this response**
        — do NOT write `autolearn-pending` and wait for the next prompt. You already have the
        `.sc` content in context. Proceed immediately:
        a. Create `.github/skills/[domain]/SKILL.md` using the skill format:
           ```
           ---
           name: [domain]
           ...
           ```
        b. Add `[domain]` to `.github/skills/skill-rules.json` with keywords from the `.sc` files
