...
process.stdin.on('end', () => {
  ...
  for (const [re, label] of BLOCKED_PATTERNS) {
    if (re.test(command)) {
      process.stdout.write('BLOCKED: Command appears to access secrets or credentials.\n');
      process.stdout.write(`Pattern matched: ${label}\n`);
      process.stdout.write('If this is intentional, run the command manually outside Claude Code.\n');
      process.exit(2);
    }
  }
  ...
  for (const [re, label] of COMMIT_SECRET_PATTERNS) {
    if (re.test(command)) {
      process.stdout.write('BLOCKED: Command may stage secret files.\n');
      process.stdout.write(`Pattern matched: ${label}\n`);
      process.stdout.write('Use specific file names with git add instead.\n');
      process.exit(2);
    }
  }
