#!/usr/bin/env node
'use strict';
// skill-detector-copilot.cjs - UserPromptSubmit hook for deterministic skill pre-filtering
// Copilot port of .claude/hooks/skill-detector.cjs
//
...
//
// Paths (relative to .github/hooks/ install location):
//   ROOT_DIR = two levels up from __dirname (.github/hooks/ -> project root)
//   rulesPath: ROOT_DIR/.github/skills/skill-rules.json
//   skillDir:  ROOT_DIR/.github/skills/
//   Skills are stored as directories: skillDir/<name>/SKILL.md

const fs = require('fs');
const path = require('path');

const ROOT_DIR = path.resolve(__dirname, '../..');
const rulesPath = path.join(ROOT_DIR, '.github', 'skills', 'skill-rules.json');
const skillDir  = path.join(ROOT_DIR, '.github', 'skills');
const pendingFile = path.join(ROOT_DIR, '.github', 'autolearn-pending');
const usageFile   = path.join(ROOT_DIR, '.github', 'skill-usage.json');
...
process.stdin.on('end', () => {
  ...
  // Copilot skill content is stored at skillDir/<name>/SKILL.md (directory-based)
  if (matched.length > 0 && !synthesisPending) {
    ...
    if (parts.length > 0) {
      ...
      process.stdout.write(`The following ${label} automatically activated based on your prompt. Apply the domain knowledge in your response:\n\n`);
    ...
    if (domain) {
      ...
      if (scFiles.length > 0) {
        ...
        const lines = [
          ...
          ``,
          `Steps:`,
          `1. Synthesize the .sc files below into .github/skills/${domain}/SKILL.md`,
          ...
          `2. Add '${domain}' to .github/skills/skill-rules.json with keywords extracted from the .sc files`,
          ...
          `      but no skill file yet in .github/skills/`,
          `   b. If another domain found: write it to .github/autolearn-pending (queue next synthesis)`,
          `   c. If none: delete .github/autolearn-pending`,
