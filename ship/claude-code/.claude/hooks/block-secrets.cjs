...
process.stdin.on('end', () => {
  ...
  for (const [re, label] of BLOCKED_PATTERNS) {
    if (re.test(command)) {
      block(`BLOCKED: Command appears to access secrets or credentials. Pattern matched: ${label}. If intentional, run the command manually outside the AI assistant.`);
    }
  }
  ...
  for (const [re, label] of COMMIT_SECRET_PATTERNS) {
    if (re.test(command)) {
      block(`BLOCKED: Command may stage secret files. Pattern matched: ${label}. Use specific file names with git add instead.`);
    }
  }
